...
function colorShip(verdict, glyph, colors) {
    switch (verdict) {
        case 'ready':
            return green(glyph);
        case 'warning':
...
function formatCiCounts(ci, spacing, colors) {
    if (ci.total === 0) {
        return null;
    }
    const parts = [];
    if (ci.success > 0)
        parts.push(glyphPair(green('✓'), String(ci.success), spacing));
    if (ci.failure > 0)
        parts.push(glyphPair(critical('✗', colors), String(ci.failure), spacing));
    if (ci.pending > 0)
        parts.push(glyphPair(dim('◐'), String(ci.pending), spacing));
    if (parts.length === 0)
        return null;
    return joinGlyphPairs(parts, spacing);
}
function formatErrorHint(error, colors) {
    switch (error) {
        case 'gh-unauthed':
...
export function renderPrStatusLine(ctx) {
